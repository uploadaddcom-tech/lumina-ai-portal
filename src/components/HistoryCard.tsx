...
export function HistoryCard({ item, styleLabel, lang, onRestore, onDelete }: HistoryCardProps) {
  ...
  return (
    <div 
      className="bg-transparent border-b border-slate-100 dark:border-white/10 pb-4 last:border-b-0 flex flex-col md:flex-row md:items-center justify-between gap-3 text-left"
    >
      <div className="flex items-center gap-3 min-w-0 flex-1">
        ...
        <div className="min-w-0 flex-1 space-y-1">
          <div className="flex items-center gap-2 flex-wrap min-w-0">
            <h4 className="text-xs font-bold text-slate-900 dark:text-white line-clamp-1 truncate flex-1 min-w-0">{item.fileName}</h4>
          ...
          <div className="flex items-center gap-2 flex-wrap text-[10px]">
            <span className="text-[9px] text-slate-500 dark:text-slate-400 font-bold bg-slate-100 dark:bg-white/5 px-1.5 py-0.5 rounded border border-slate-200 dark:border-white/5 leading-none">
              {styleLabel}
            </span>
